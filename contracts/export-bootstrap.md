...
## Validation
A bootstrap is healthy only if a complete novice can initialize from the one-line remote instruction when web access exists, or from the standalone bootstrap when it does not, decline cloud storage, and still receive phased onboarding and full advisory behavior. Remote distribution additionally requires short-link resolution to the canonical raw Production bootstrap and version parity with `releases/LATEST.json`. Session-provenance validation must prove operation without a host reference and prove that host references cannot merge accounts, bypass `active_account_id`, reorder recovery, or act as idempotency keys.
